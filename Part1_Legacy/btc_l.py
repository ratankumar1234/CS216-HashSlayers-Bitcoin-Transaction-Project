...
RPC_PORT = 18443
RPC_HOST = "172.19.48.1"
WALLET_NAME = "testwallet" 

RPC_NODE_URL = f"http://{RPC_USER}:{RPC_PASS}@{RPC_HOST}:{RPC_PORT}"
RPC_WALLET_URL = f"{RPC_NODE_URL}/wallet/{WALLET_NAME}"
...
def rpc(method, params=None):
    ...
    response = requests.post(RPC_WALLET_URL, json=payload)
...
def rpc_node(method, params=None):
    if params is None: params = []
    payload = {"jsonrpc": "1.0", "id": "assignment", "method": method, "params": params}
    response = requests.post(RPC_NODE_URL, json=payload)
    result = response.json()
    if result.get("error"):
        print(f"Node RPC Error in {method}: {result['error']}")
        return None
    return result["result"]
